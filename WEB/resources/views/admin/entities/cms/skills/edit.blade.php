        ...
        <div class="container-fluid container-fixed-lg">
            <ol class="breadcrumb">
                <li class="breadcrumb-item">
                    <a href="{{ action([\App\Http\Controllers\Admin\Cms\SkillController::class, 'index']) }}">Traits de caractère</a>
                </li>
                <li class="breadcrumb-item active">#SK{{str_pad($skill->id, 5, '0', STR_PAD_LEFT)}}</li>
            </ol>
            <form action="{{ action([\App\Http\Controllers\Admin\Cms\SkillController::class, 'update'], $skill->id) }}" method="POST"
                                    ...
                                    <div class="col-md-12">
                                        <div class="form-group {{ $errors->has('title') ? 'has-error' : '' }} form-group-default required"
                                            ...
                                            <input type="text" class="form-control" name="title"
                                                   value="{{ $skill->name }}" placeholder="Nom de la compétence"
                                                   required>
                                        </div>
                                    </div>
                                </div>
                                    ...
                                    <div class="col-md-12">
                                        <div class="form-group {{ $errors->has('title') ? 'has-error' : '' }} form-group-default required"
                                            ...
                                            <textarea type="text" class="form-control editor" name="content"
                                                      ...
                                                      style="height: 50px;"
                                                      required>{{$skill->content}}</textarea>
                                        </div>
                                    </div>
                                </div>
                            </div>
