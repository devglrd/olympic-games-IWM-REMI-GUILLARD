...
<link href="{{ asset('pages-assets/plugins/bootstrap-daterangepicker/daterangepicker-bs3.css') }}" rel="stylesheet" type="text/css" media="screen">
<link href="{{ asset('pages-assets/plugins/bootstrap-timepicker/bootstrap-timepicker.min.css') }}" rel="stylesheet" type="text/css" media="screen">
@stop
...
<div class="content sm-gutter">
    <div class="container-fluid container-fixed-lg">
        <ol class="breadcrumb">
            ...
            <li class="breadcrumb-item active">#AC{{str_pad($activity->id, 5, '0', STR_PAD_LEFT)}}</li>
        </ol>
        <form action="{{ action([\App\Http\Controllers\Admin\Cms\ActivityController::class, 'update'], $activity->id) }}" method="POST" data-toggle="validator" enctype="multipart/form-data">
            {{ method_field('put') }}
            {{ csrf_field() }}
            <div class="row">
                            ...
                            <div class="row clearfix">
                                <div class="col-md-12">
                                    <div class="form-group {{ $errors->has('title') ? 'has-error' : '' }} form-group-default required" aria-required="true">
                                        <label>Nom</label>
                                        <input type="text" class="form-control" name="title" value="{{ $activity->name }}" placeholder="Nom" required>
                                    </div>
                                </div>
                            </div>
                            <div class="row clearfix">
                                <div class="col-md-12">
                                    <div class="form-group {{ $errors->has('title') ? 'has-error' : '' }} form-group-default required" aria-required="true">
                                        ...
                                        <textarea type="text" class="form-control editor" name="content" placeholder="Description" style="height: 50px;" required>{{ $activity->content }}</textarea>
                                    </div>
                                </div>
                            </div>
                        </div>
