...
export class SportController {
  ...
  async index(@Req() req, @Res() res) {
    if (req.query.filterType && req.query.filterType === 'events') {
      const sports = await this.sportService.filterSport();
      const al = await Promise.all(
        sports.map((e) => Sport.findOne({ where: { id: e.id } })),
      );
      res.send({
  ...
  async delete(@Req() req, @Res() res) {
    await this.sportService.delete(req.params.slug);
    return res.send({
      message: 'Success',
    });
  }
