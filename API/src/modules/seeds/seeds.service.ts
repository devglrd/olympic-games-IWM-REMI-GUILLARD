...
export class SeedsService {
    ...
    private async seedSport() {
        const sportsData = await this.httpService.get('https://parseapi.back4app.com/classes/Listsportstokyolympic', {
            headers: {
        ...
        const sports = sportsData.data.results.map(async (e) => {
            const sportFromDB = await this.sportRepository.findOne({where: {slug: slugify(e.Sport)}});
    ...
    private async seedEvent() {
        const eventData = await this.httpService.get('https://parseapi.back4app.com/classes/Listsportstokyolympic', {
            headers: {
                'X-Parse-Application-Id': 'Dwco6bapxxssPbLehcLJ244hWjXYhrHPGdyONZNm', // This is the fake app's application id
                'X-Parse-Master-Key': 'PfOgFvxS79kRM1v7guxCyXLw93uSk348Q376mSjs', // This is the fake app's readonly master key
            }
        }).toPromise();
        const events = eventData.data.results.map(async (e) => {
            const sport = await this.sportRepository.findOne({where: {slug: slugify(e.Sport)}});
            if (sport) {
                const event = new EventCategory();
                event.name = e.Programme;
                event.slug = slugify(e.Programme);
                event.type = e.Programme.includes('(Men/Women)') ? Type.MenWomen : (e.Programme.includes('(Men)') ? Type.Men : (e.Programme.includes('(Women)') ? Type.Women : Type.Unknow))
                event.sport = sport;
                await event.save();
                return event;
            }
